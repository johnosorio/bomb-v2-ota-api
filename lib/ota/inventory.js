...
export function userScopedClient(config, authorization, fetchImpl = globalThis.fetch, conflictCode = "DEVICE_CONFLICT") {
  ...
  async function request(path, options = {}, auth = false) {
    ...
    if (!response.ok) {
      ...
      if (!auth && response.status < 500) {
        ...
        if (payload?.code === "23505") throw new OtaError(409, conflictCode);
...
export function inventory(config, authorization, fetchImpl = globalThis.fetch) {
  const { request, authenticate } = userScopedClient(config, authorization, fetchImpl);
  return {
    authenticate,
